...
4. **Test the Policy with Kyverno:**
   ...
   kyverno-json scan --payload payload.json --policy policy.yaml
    ...
    ```
    kyverno-json scan --policy check-awsvpc-network-mode.yaml --payload test/good-payload.json --bindings test/binding.yaml
    ```
    ...
    ```
    kyverno-json scan --policy check-awsvpc-network-mode.yaml --payload test/bad-payload.json --bindings test/binding.yaml
    ```
    ...
    Pre processing ...
    Running ( evaluating 1 resource against 1 policy ) ...
    - check-awsvpc-network-mode / check-awsvpc-network-mode /  FAILED
    -> ECS services and tasks are required to use awsvpc network mode.
    -> all[0].check.~.(planned_values.root_module.resources[?type=='aws_ecs_task_definition'].values)[0].network_mode: Invalid value: "bridge": Expected value: "awsvpc"
    Done
    ```
